      ...
      '<div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:22px;overflow:hidden;box-shadow:0 10px 32px rgba(20,20,20,.08)">' +
        ...
        '<div style="padding:26px 30px 30px;background:#FFF8E7;text-align:center;border-top:1px solid #FFBC0D">' +
          ...
          '<p style="margin:0;font-size:12.5px;color:#9A9A9A;line-height:1.8">' +
            ...
            '<a href="tel:+918287124651" style="color:#DB0007;text-decoration:none">+91 82871 24651</a><br>' +
            '<a href="tel:+919810005956" style="color:#DB0007;text-decoration:none">+91 98100 05956</a></p>' +
...
function customerEmail(o) {
  ...
  var inner =
    '<div style="padding:34px 30px 10px">' +
      ...
      '</div>' +
      '<p style="margin:-14px 0 24px;font-size:12px;color:#9A9A9A;text-align:center">Your invoice is attached as a PDF.</p>' +
...
async function send(resendKey, to, subject, html, replyTo, attachments) {
  var payload = { from: FROM, to: [to], subject: subject, html: html, reply_to: replyTo };
...
async function sendOrderEmails(resendKey, o, opts) {
  opts = opts || {};
  // Build the PDF invoice once; if generation fails, send emails without it.
  var attachments;
  try {
    var pdf = invoice.buildInvoicePdf(o);
    if (pdf && pdf.length) attachments = [{ filename: "TheMakhana-Invoice-" + (o.order_no || "order") + ".pdf", content: pdf.toString("base64") }];
  } catch (e) { attachments = undefined; }
  var tasks = [];
  if (o.email) tasks.push(send(resendKey, o.email, "Your The Makhana order is confirmed — #" + (o.order_no || ""), customerEmail(o), OWNER, attachments).then(function (r) { return ["customer", r]; }));
  if (opts.notifyOwner !== false) tasks.push(send(resendKey, OWNER, "New order — #" + (o.order_no || "") + " (₹" + (o.total || 0) + ")", ownerEmail(o), o.email || undefined, attachments).then(function (r) { return ["owner", r]; }));
